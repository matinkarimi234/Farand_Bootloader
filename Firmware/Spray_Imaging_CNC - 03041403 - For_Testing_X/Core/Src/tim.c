...
{	
	if(htim->Instance == TIM2) // This interrupt can be used for Alarming (32Hz)
	{
		counter_toggle ++;
    ...
    Update_Alarm();	
		...
		//Update_X_Axis_State_Machine();
		
	}		
	
	if(htim->Instance == TIM1) // (100Hz)
	{	
    
	}	

	if(htim->Instance == TIM4) // (2kHz)
	{	
		//HAL_GPIO_TogglePin(Z_Driver_Pulse_GPIO_Port, Z_Driver_Pulse_Pin);
		Update_State_Machine(&X_Axis);
		Update_State_Machine(&Y_Axis);
		Update_State_Machine(&Z_Axis);
	}		
}
